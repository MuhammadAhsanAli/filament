...
@php
    ...
    $itemLabelClasses = 'fi-breadcrumbs-item-label text-sm font-medium text-black-500 dark:text-gray-400';
    $activeItemLabelClasses = 'fi-breadcrumbs-item-label text-sm font-medium text-gray-500 dark:text-white';
@endphp
    ...
    <ol class="fi-breadcrumbs-list flex flex-wrap items-center gap-x-2">
        @foreach ($breadcrumbs as $url => $label)
            <li class="fi-breadcrumbs-item flex items-center gap-x-2">
                ...
                @if ($loop->last)
                    <span class="{{ $activeItemLabelClasses }}">
                    {{ $label }}
                </span>
                @else
                    @if (is_int($url))
